...
The most significant correlation is between Year and GPN with 1.00
...
max(vif_model)
```
The variable with the largest VIF is GNP with a value of `r max(vif_model)` and **GNP.deflator**, **GNP**, **Unemployed**, **Population**, and **Year** show multicollinearity because their values are largest than 5

**(c)** What proportion of the observed variation in `Population` is explained by a linear relationship with the other predictors?

```{r}
populatoin_model = lm(Population ~ . -Employed, data = longley)
summary(populatoin_model)$r.squared 
```
The other predictors explain `r summary(populatoin_model)$r.squared * 100`% of the variation in Population

**(d)** Calculate the partial correlation coefficient for `Population` and `Employed` **with the effects of the other predictors removed**.

```{r}
model = lm(Employed ~ . -Population, data = longley)
model2 = lm(Population ~ . -Employed, data = longley)
part_coef = cor(resid(model2), resid(model))
part_coef
```
...
sign_model = lm(Employed ~ Unemployed + Armed.Forces + Year, data = longley)
vif_r = vif(sign_model)
vif_r
max(vif_r)
```
The largest VIF is Unemployed with: `r max(vif_r)`, and they do not suggest multicollinearity. 

**(f)** Use an $F$-test to compare the models in parts **(b)** and **(e)**. Report the following:

```{r}
test_m = anova(sign_model, model)
test_m
test_m$F[2]
```


- The null hypothesis is: $\beta gnp.def$ = $\beta gnp$ = $\beta population$ = 0
- The test statistic is `r test_m$F[2]`
- The distribution of the test statistic under the null hypothesis is between 3 and 9.
- The p-value is: `r test_m[2, "Pr(>F)"]`
- A decision is: we Fail to reject the null hypothesis
- Which model you prefer, **(b)** or **(e)**: the ANOVA test result told us that the small model is better which is the **e**
...
```{r fig.height=5, fig.width=10}
par(mfrow = c(1,2))
plot_fitted_resid(sign_model)
plot_qq(sign_model)
```

By the plot, we can appreciate that any assumption is violated  
***
...
all_model = lm(Balance ~ ., data = Credit)

best_model = step(all_model, direction = "backward")

outliers = as.vector(as.integer(names(rstandard(best_model)[abs(rstandard(best_model)) > 2])))
high_influence = as.vector(which(cooks.distance(best_model) > 4 / length(cooks.distance(best_model))))
x1 = c(outliers, high_influence)
fixed_model = Credit[-x1,]

mod_a = lm(Balance ~ Income + Limit + Cards + Age + Student, data = fixed_model)
...
```
This is the best model I found after look for all tipes of model. 
...
```{r fig.height=5, fig.width=10}
best_model2 = lm(price ~ beds + sqft + type + latitude + longitude, data = sac_trn_data)

outliers = as.vector(as.integer(names(rstandard(best_model2)[abs(rstandard(best_model2)) > 2])))
high_influence = as.vector(which(cooks.distance(best_model2) > 4 / length(cooks.distance(best_model2))))

remove_all = c(outliers, high_influence)

clean_data = sac_trn_data[-remove_all,]

best_model3 = lm(price ~ beds + sqft + type + limits, data = clean_data)

get_loocv_rmse(best_model3)
...
```{r message=TRUE, warning=TRUE}
library(MLmetrics)
predicted_values = predict(best_model3, newdata = sac_tst_data)
avg_pct_err = mean(abs((predicted_values - sac_tst_data$price)/predicted_values))* 100

MAPE(sac_tst_data$price, predicted_values) * 100
avg_pct_err
```
```{r fig.height=5, fig.width=10}
...
I think this model is not useful because an average error of  `r avg_pct_err`% which is high in case of predicting the value of a property, we can also see in the plot how we have some crazy prediction vs. real price.  
...
x_9  = runif(n, 0, 10)
x_10 = runif(n, 0, 10)
false_negative_aic = rep(0, 300)
false_positive_aic = rep(0, 300)
false_negativ_bic = rep(0, 300)
false_positive_bic = rep(0, 300)
...
for(i in 1:300){
  ...
  back_aic_1 = step(fit, direction = "backward");
  back_bic_1 = step(fit, direction = "backward", k = log(n1))
  
  false_negative_aic[i] = sum(!(signif %in% names(coef(back_aic_1))))
  false_positive_aic[i] = sum(names(coef(back_aic_1)) %in% not_sig)
  
  false_negativ_bic[i] = sum(!(signif %in% names(coef(back_bic_1))))
  false_positive_bic[i] = sum(names(coef(back_bic_1)) %in% not_sig)
}
```
...
```{r}
library(knitr)
aic_neg_mean = mean(false_negative_aic)
aic_pos_mean = mean(false_positive_aic)
bic_neg_mean = mean(false_negativ_bic)
bic_pos_mean = mean(false_positive_bic)


df = data.frame("AIC" = c("False Negative" = aic_neg_mean,"False Positive" = aic_pos_mean
  ), "BIC" = c("False Negative" = bic_neg_mean, "False Positive" = bic_pos_mean ))

kable(df, caption = "False Comparison table")
```

The system shows that False-negative has a perfect average of 0, but that did not happen with the False Positive, but with the result, we will go with the BIC model
...
df2 = data.frame("AIC" = c("False Negative" = aic_neg_mean,"False Positive" = aic_pos_mean
  ), "BIC" = c("False Negative" = bic_neg_mean, "False Positive" = bic_pos_mean ))

kable(df2, caption = "False Comparison table")
```

We can see in our table that the BIC was better than AIC but not the best result. The differences between models **a** & **b** could be related to the correlation between x_8, x_9 and x_10 that exist in model **b** that are not in model **a**, So in Average BIC preformed better than AIC for this model
